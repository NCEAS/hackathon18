...
ToDos: Check `googledrive` package to access files from Team Drive
...
### Fixing different date formats in the `description` file
...
- Every time the vendor is a person_id and not a hotel name, we assume it's food. It could also be taxi, baggage fees. We assume that it's money paid in SB.  It would be nice if there were a way to identify it more in GUS. 
...
- GUS truncate the `Description` field at 60 characters. Could we use the `Notes` field? 

- Standardizing date format

- filter out `Description` without numeric values; will allow to check that `PAR` for none participant
